...
## 📖 项目简介
支持四种主流 LLM)、专注于软件开发的AI Code Agent ，能够自主完成从单文件
脚本到多模块系统的全栈开发任务，并具备基于反馈的容错恢复能力。通过MCP协议提升实现零代码添加
多语言工具，采用docker沙盒隔离提高核心安全性。 所有功能均通过现代化的Web UI呈现，实现毫秒级的实时流式反馈，极大提升了用户体验。

**⭐ v1.3.0 新增：现代化 Web 界面！** - 提供美观的 Next.js + Flask 前后端分离架构，支持实时对话、可视化配置、工具管理等功能。

**⭐ v1.4.0 新增：安全与协作！** - 引入 Docker 沙盒隔离环境，保障代码执行安全；新增 Human-in-the-Loop 机制，支持人工审核与修改执行计划。

### 🎯 核心能力
- 📋 **任务规划** - 执行前生成结构化计划，减少无效操作 30-50% (v1.1.0)
- 🤝 **人机协作** - **[新增]** 执行计划需人工确认，支持自然语言修改计划，精准控制执行流 (v1.4.0)
- 🛡️ **安全沙盒** - **[新增]** 所有代码执行在 Docker 容器中隔离运行，彻底防止误删文件或系统破坏 (v1.4.0)
...
- **持久化配置** - 自定义配置永久保存
- 
### 🛡️ 安全与协作 (Security & Collaboration) ⭐ v1.4.0 新增
#### 📦 Docker 安全沙盒 (Sandbox Isolation)
为了防止 Agent 生成的恶意代码或误操作（如 `rm -rf /`）破坏您的主机环境，本项目内置了 Docker 沙盒机制。
- **环境隔离**：`run_python` 和 `run_shell` 工具将自动在一个独立的 Docker 容器（默认 `python:3.9-slim`）中运行。
- **文件映射**：自动挂载 `workspace` 目录，确保 Agent 生成的文件可在本地查看，但无法访问主机其他目录。
- **自动降级**：检测到 Docker 未启动时，自动提示并降级为本地虚拟环境执行（不推荐用于不可信任务）。
- **生命周期管理**：程序启动时自动创建沙盒，退出时自动销毁，不残留垃圾容器。

#### 🤝 人机交互与计划确认 (Human-in-the-Loop)
不再让 Agent "黑盒" 执行！在 ReAct 循环中增加了人工决策环节：
1. **生成计划**：Agent 接收任务后，首先生成详细的执行步骤。
2. **人工审核**：系统暂停并展示计划，等待用户确认。
   - 输入 `y`：批准计划，开始自动执行。
   - 输入 `n`：拒绝并退出。
   - **输入修改意见**：直接用自然语言（如"不要用 Pygame，改用 Curses"），Agent 会根据您的反馈**动态修正计划**。
3. **执行反馈**：每一步执行结果都清晰可见，确保任务按预期进行。

## 📋 前置要求
- **Python 3.7+** （推荐 3.9 或更高版本）
- **Docker Desktop** (强烈推荐) - 用于启用安全沙盒功能。若未安装，系统将以非隔离模式运行。
...
# DeepSeek（默认）
DEEPSEEK_API_KEY=sk-2c28dd6d37be46c7b00299751365166f
...
# 智能体会记住 test.py 的上下文
```
### 示例 8: 人机协作与安全执行 ⭐ v1.4.0

执行一个可能涉及系统操作的任务：

```bash
python main.py "写一个脚本扫描当前系统的内核版本并保存到 info.txt"
## ⚙️ 命令行参数
...
📋 待确认的执行计划：
计划进度：0/3 步骤已完成
○ 步骤 1: run_shell - 执行 uname -a 获取内核信息
○ 步骤 2: create_file - 创建 info.txt 保存结果
○ 步骤 3: task_complete - 任务完成

--------------------------------------------------
👉 请确认计划 (y=执行 / n=退出 / 输入其他文字作为修改意见): 
> 先不要保存，直接打印出来让我看看
...
📋 新的执行计划：
○ 步骤 1: run_shell - 执行 uname -a 并打印输出
○ 步骤 2: task_complete - 任务完成
