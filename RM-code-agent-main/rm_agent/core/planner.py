...
"""
    ...
    def get_progress(self) -> str:
        ...
        # 添加到 planner.py 的 TaskPlanner 类中
    def revise_plan(self, task: str, current_plan: List[PlanStep], feedback: str) -> List[PlanStep]:
            """
            根据用户反馈修改计划 (Human-in-the-loop)
            """
            # 将当前计划转换为文本摘要
            plan_summary = "\n".join(
                [f"步骤 {s.step_number}: {s.action} - {s.reason}" for s in current_plan]
            )

            prompt = f"""用户对当前的执行计划提出了修改意见。请根据反馈重新生成计划。
    ...
    请生成一个新的、符合用户要求的执行计划。返回 JSON 格式（与之前一致）：
    {{
      "plan": [
        {{"step": 1, "action": "工具名称", "reason": "..."}},
        ...
      ]
    }}
    """
