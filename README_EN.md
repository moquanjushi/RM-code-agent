...
**⭐ v1.3.0 New: Modern Web Interface!** - Provides a beautiful Next.js + Flask architecture with real-time chat, visual config, and tool management.

**⭐ v1.4.0 New: Security & Collaboration!** - Introduces Docker Sandbox isolation for safe code execution; adds Human-in-the-Loop mechanism for manual review and modification of execution plans.

### 🎯 Core Capabilities
- 📋 **Task Planning** - Generate structured plans before execution, reduce ineffective operations by 30-50% (v1.1.0)
- 🤝 **Human-Machine Collab** - **[New]** Execution plans require manual confirmation, supporting natural language modification for precise control (v1.4.0)
- 🛡️ **Security Sandbox** - **[New]** All code executes in isolated Docker containers, preventing system damage (v1.4.0)
...
- 🌐 **Modern Web UI** - Next.js + Flask architecture, real-time communication, beautiful and easy to use (v1.3.0)
...
### 🛡️ Security & Collaboration (Security & Collaboration) ⭐ v1.4.0 New
#### 📦 Docker Security Sandbox (Sandbox Isolation)
To prevent malicious code or accidental operations (like `rm -rf /`) from damaging your host environment, this project has built-in Docker sandbox mechanisms.
- **Environment Isolation**: `run_python` and `run_shell` tools automatically run inside an independent Docker container (default `python:3.9-slim`).
- **File Mapping**: Automatically mounts the `workspace` directory, ensuring generated files are viewable locally but preventing access to other host directories.
- **Auto-Downgrade**: Detects if Docker is not started and prompts to downgrade to local virtual environment execution (not recommended for untrusted tasks).
- **Lifecycle Management**: Auto-creates sandbox on startup and destroys it on exit, leaving no residual containers.

#### 🤝 Human-in-the-Loop & Plan Confirmation
No more "Black Box" execution! A manual decision step is added to the ReAct loop:
1. **Plan Generation**: Agent generates detailed steps upon receiving a task.
2. **Manual Review**: System pauses and shows the plan, waiting for user confirmation.
   - Type `y`: Approve plan, start automatic execution.
   - Type `n`: Reject and exit.
   - **Input Feedback**: Use natural language (e.g., "Don't use Pygame, use Curses instead"), and the Agent will **dynamically revise the plan**.
3. **Execution Feedback**: Results of every step are clearly visible, ensuring the task proceeds as expected.

## 📋 Prerequisites

- **Python 3.7+** (Recommended 3.9 or higher)
- **Docker Desktop** (Strongly Recommended) - For enabling security sandbox features. If not installed, system runs in non-isolated mode.
...
```
### Example 8: Human Collaboration & Safe Execution ⭐ v1.4.0

Execute a task involving system operations:

```bash
python main.py "Write a script to scan the current system kernel version and save to info.txt"
...
📋 Execution Plan to Confirm:
Plan Progress: 0/3 steps completed
○ Step 1: run_shell - Execute uname -a to get kernel info
○ Step 2: create_file - Create info.txt to save results
○ Step 3: task_complete - Task complete

--------------------------------------------------
👉 Confirm plan (y=run / n=exit / input text to modify): 
> Don't save it, just print it out for me to see

🔄 Revising plan based on feedback...
✅ Plan updated.

📋 New Execution Plan:
○ Step 1: run_shell - Execute uname -a and print output
○ Step 2: task_complete - Task complete

👉 Confirm plan: y
✅ Plan confirmed, executing in sandbox...
