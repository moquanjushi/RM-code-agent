...
# [新增] 导入沙盒控制函数
from dm_agent.tools import start_sandbox, stop_sandbox
...
def main(argv: Any = None) -> int:
    ...
    # === [新增] 启动 Docker 沙盒 ===
    try:
        print(f"{Fore.CYAN}📦 正在启动 Docker 沙盒...{Style.RESET_ALL}")
        start_sandbox()
        print(f"{Fore.GREEN}✓ 沙盒启动成功，代码将在隔离环境中运行{Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}✗ 沙盒启动失败：{e}{Style.RESET_ALL}", file=sys.stderr)
        print(f"{Fore.YELLOW}请确保 Docker Desktop 已安装并正在运行。{Style.RESET_ALL}", file=sys.stderr)
        return 1
    # ============================
    ...
    try:
        ...
        # === [新增] 关闭 Docker 沙盒 ===
        # 无论程序是正常结束还是报错退出，都确保清理容器
        print(f"\n{Fore.CYAN}正在关闭 Docker 沙盒...{Style.RESET_ALL}")
        stop_sandbox()
        print(f"{Fore.GREEN}✓ 沙盒已关闭{Style.RESET_ALL}")
        # ============================
